...
create_instance("TextLabel", {
    ...
    Font = Enum.Font.Arcade, Text = "V2.0.0",
...
local search_box = create_instance("TextBox", {
    ...
    PlaceholderText = "SEARCH SCRIPT...", Text = "",
...
result_count_label = create_instance("TextLabel", {
    ...
    Position = UDim2.new(0, 10, 0, 116), Size = UDim2.new(1, -20, 0, 14),
...
local clear_cache_btn = create_instance("TextButton", {
    ...
    BorderSizePixel = 0, Position = UDim2.new(0, 10, 0, regex_y + 36),
    Size = UDim2.new(1, -20, 0, 22), Font = Enum.Font.Arcade,
    Text = "CLEAR DECOMPILE CACHE", TextColor3 = Color3.fromRGB(255,255,255),
            ...
            if active_search_terms and #active_search_terms > 0 then
                for k, line_text in ipairs(chunk_lines) do
                    for _, term in ipairs(active_search_terms) do
                        local pattern = setting_use_regex and term or case_insensitive_pattern(escape_pattern(term))
                        local init = 1
                        while true do
...
local sort_labels = {matches="SORT:MATCH", name="SORT:NAME", path="SORT:PATH"}
local sort_idx = 1
bind_tap(sort_button, function()
    sort_idx = sort_idx % #sort_modes + 1
    setting_sort_mode = sort_modes[sort_idx]
    sort_button.Text = sort_labels[setting_sort_mode]
    render_results()
end)
    ...
    search_thread = task.spawn(function()
        ...
        for i = 1, #all_scripts do
            ...
            for _, term in ipairs(terms) do
                local pattern = setting_use_regex and term or escape_pattern(term)
                if not string.find(name_lower, pattern) and not string.find(bytecode_lower, pattern) then
                    match_all = false
                    break
                end
            end
...
local original_main_size = UDim2.new(0, 420, 0, 370)
local original_main_pos = UDim2.new(0.5, -210, 0.5, -185)
...
bind_tap(hide_button, function()
    if not is_collapsed then
        ...
        tween_service:Create(resize_handle, TweenInfo.new(0.25), {Size = UDim2.new(0, 0, 0, 0)}):Play()
        local tw = tween_service:Create(main_gui, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Size = UDim2.new(0, 0, 0, 0), Position = center_pos
        })
        tw:Play()
        tw.Completed:Connect(function()
            ...
            tween_service:Create(floating_hide, TweenInfo.new(0.2), {
                Size = UDim2.new(0, 40, 0, 40), Position = UDim2.new(0, fx, 0, fy)
            }):Play()
        end)
    end
end)
...
local floating_drag_start_pos, floating_start_pos
...
user_input_service.InputChanged:Connect(function(input)
    if is_dragging_floating and input.UserInputType == Enum.UserInputType.Touch then
        ...
        if delta.Magnitude > floating_max_dist then floating_max_dist = delta.Magnitude end
...
user_input_service.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch then
        if is_dragging_floating then
            ...
            if floating_max_dist < 2 and is_collapsed then
                is_collapsed = false
                ...
                local tx, ty = clamp_pos(center_x - original_main_size.X.Offset/2, center_y - original_main_size.Y.Offset/2, original_main_size.X.Offset, original_main_size.Y.Offset)
                original_main_pos = UDim2.new(0, tx, 0, ty)
                local tw = tween_service:Create(floating_hide, TweenInfo.new(0.2), {Size = UDim2.new(0,0,0,0), Position = center_pos})
                tw:Play()
                tw.Completed:Connect(function()
                    ...
                    tween_service:Create(resize_handle, TweenInfo.new(0.25), {Size = UDim2.new(0,20,0,20)}):Play()
                    tween_service:Create(main_gui, TweenInfo.new(0.3), {Size = original_main_size, Position = original_main_pos}):Play()
                end)
            end
        end
    end
...
bind_tap(close_button, function()
    ...
    for _, child in ipairs(children) do
        if child:IsA("GuiObject") then
            ...
            tween_service:Create(child, TweenInfo.new(0.25), props):Play()
        end
    end
    task.delay(0.3, function()
        tween_service:Create(main_gui, TweenInfo.new(0.25), {
            Size = UDim2.new(0, main_gui.AbsoluteSize.X, 0, 0),
            BackgroundTransparency = 1
        }):Play()
        task.delay(0.28, function() screen_gui:Destroy() end)
    end)
end)
...
local main_drag_start_pos, main_start_pos
...
user_input_service.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch then is_dragging_main = false end
end)
...
user_input_service.InputChanged:Connect(function(input)
    if is_resizing and input.UserInputType == Enum.UserInputType.Touch then
        ...
        local new_y = math.clamp(start_size.Y.Offset + delta.Y, 220, max_h)
...
print("MEGGD Script Scanner Enhanced V1.3.1 - Loaded")
